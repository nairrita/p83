...
export default class MyFoodDonationScreen extends Component {
   constructor(){
     super()
     this.state = {
       ...
       snapshot.forEach((doc) => {
         ...
         if(requestStatus === "Food Sent"){
           message = this.state.donorName + " sent you food"
            ...
            message =  this.state.donorName  + " has shown interest in donating the food"
         }
       ...
       rightElement={
           <TouchableOpacity
            style={[
              styles.button,
              {
                backgroundColor : item.request_status === "Food Sent" ? "#0099ff" : "#ff00c3"
              }
            ]}
            onPress = {()=>{
              this.sendFood(item)
